...
def identify_neoantigens(df):
    ...
    class_I = [i for i in df_neoantigen["HLA Allele"]
               if any(x in i for x in ["HLA-A", "HLA-B", "HLA-C"])]
    class_II = [i for i in df_neoantigen["HLA Allele"]
                if any(x in i for x in ["DP", "DQ", "DR"])]
    index_list_classI = []
    index_list_classII = []
    for row_index, row in df_neoantigen.iterrows():
        hla, wt_score, fold_change, mutation_position, seq_length = (
            row["HLA Allele"], row["Median WT Score"], row["Median Fold Change"], row["Mutation Position"], row["Peptide Length"])
...
def score_intervals(df, message):
    ...
    df_med_score = df.loc[(df["Median MT Score"] > 50) &
                          (df["Median MT Score"] <= 500)]
    df_high_score = df.loc[(df["Median MT Score"] > 500)
                           & (df["Median MT Score"] <= 1000)]
    score = [(f'{message} Score < 50nM', len(df_low_score)), (f'{message} Score 50-500nM',
                                                              len(df_med_score)), (f'{message} Score 500-1000nM', len(df_high_score))]
...
def find_min_prediction(df, hla_class):
    ...
    if len(df) > 0:
        min_score_line = df[df["Median MT Score"]
                            == df["Median MT Score"].min()]
        min_dict = {x: min_score_line[x].values[0] for x in columns}
        output_df = pd.DataFrame(min_dict, index=[0])
        output_df.rename(columns={x: y for x, y in zip(
...
def length_count(df, message):
    ...
    if len(df) > 0:
        ...
        len_dict = {f'{message} Length {x}': len_seqs.count(
            x) for x in set(len_seqs)}
        len_df = pd.DataFrame(len_dict, index=[0])
    else:
        len_df = pd.DataFrame(
            {f'{message} Length {x}': "NA" for x in peptide_len}, index=[0])
    return len_df
...
output_file = '/Users/anamikabasu/Code/Database/Neoantigens/filtered.csv'
...
file1 = "chr21-g.6484823_6484825delTCC_TUMOR.all_epitopes.tsv"
df_input = pd.read_csv(
    '/Users/anamikabasu/Code/Database/Neoantigens/chr21-g.6484823_6484825delTCC_TUMOR.all_epitopes.tsv', sep='\t')
# constant info per file
df_constant = df_input[["Chromosome", "Start", "Stop", "Reference", "Variant", "Transcript",
                        "Ensembl Gene ID", "Variant Type", "Mutation", "Protein Position", "Gene Name", "HGVSp"]]
constant_line = df_constant.drop_duplicates()
# unique info per line
df_variable = df_input[["HLA Allele", "Peptide Length", "Sub-peptide Position", "Mutation Position",
                        "MT Epitope Seq", "WT Epitope Seq", "Median MT Score", "Median WT Score", "Median Fold Change"]]
df_variable.loc[df_variable["Median MT Score"] <= 50]
df_variable = df_variable.drop_duplicates()
df_classI = df_variable.loc[df_variable["HLA Allele"].str.contains(
    "HLA-A|HLA-B|HLA-C")]
df_classII = df_variable.loc[~df_variable["HLA Allele"].str.contains(
    "HLA-A|HLA-B|HLA-C")]
...
classI_neo_pred = parse_predictions(
    neo_classI, ["Neo ClassI Predictions", "Neo ClassI"])
classII_pred = parse_predictions(
    df_classII, ["ClassII Predictions", "ClassII"])
classII_neo_pred = parse_predictions(
    neo_classII, ["Neo ClassII Predictions", "Neo ClassII"])
...
final_df_list = [file_info, constant_line, total_pred, classI_pred, classI_neo_pred, classII_pred,
                 classII_neo_pred, total_scores, classI_scores, classII_scores, classI_min, classII_min, total_len, neo_len]
